...
function Sidebar() {
    ...
    function getRooms() {
        fetch("https://mernchatapp-backend-3gk2.onrender.com/rooms")
            .then((res) => res.json())
    ...
    return (
        <>
            <h2>Available rooms <FaUsers className="FaUsers-icon"/></h2>
            <ListGroup>
                {rooms.map((room, idx) => (
            ...
            <h2>Members <FaUserSecret className="FaUserSecret-icon"/></h2>
            {members.map((member) => (
